

![][image1]

**User Acceptance Testing**  
Dana Kripto Indonesia Versi 1.0

| Disiapkan Oleh | : | Fiky Anggra Sepriaji |
| :---- | :---- | :---- |
| Tanggal | : | 08 Agustus 2025 |
| Nomor Dokumen | : |  |

[**1\. LATAR BELAKANG PROYEK	3**](#heading=)

[**2\. TUJUAN PROYEK	3**](#heading=)

[**3\. SOLUTION OVERVIEW PROYEK	3**](#heading=)

[**A. Ruang Lingkup	3**](#heading=)

[**B. Batasan Dan Asumsi	3**](#heading=)

[**4\. TUJUAN USER ACCEPTANCE TEST	3**](#heading=)

[**5\. LINGKUNGAN USER ACCEPTANCE TEST	3**](#heading=)

[**Berisi environment testing beserta dengan IP Address. Contoh: Staging Environment 192.168.100.1	3**](#heading=)

[**6\. PERSIAPAN PENGUJIAN**](#heading=)  
[**Berisi persiapan yang perlu dilakukan sebelum memulai User Acceptance TEST misalnya adanya pelatihan pengguna	3**](#heading=)

[**7\. SKENARIO DAN HASIL PENGUJIAN	3**](#heading=)

[**8\. KRITERIA KEBERHASILAN	3**](#heading=)

[**9\. PENCATATAN MASALAH DAN TINDAK LANJUT**](#heading=)  
[**Berisi mekanisme pelaporan untuk mencatat masalah atau bug yang ditemukan selama pengujian.	4**](#heading=)

[**10\. EVALUASI DAN PENILAIAN	4**](#heading=)

[**11\. LAMPIRAN	4**](#heading=)

# 

# 

# 

1. # **LATAR BELAKANG PROYEK**

   Sesuai yang tertera pada dokumen BRD \[Nomor BRD\]

2. # **TUJUAN PROYEK**

   Sesuai yang tertera pada dokumen BRD \[Nomor BRD\]

3. # **SOLUTION OVERVIEW PROYEK**

1. ## **Ruang Lingkup**

   Cakupan sistem yang dikembangkan yakni

2. ## **Batasan Dan Asumsi**

   Fokus pengujian pada aspek yakni

4. # **TUJUAN USER ACCEPTANCE TEST**

   berisi tujuan dari User Acceptance Test, termasuk verifikasi bahwa sistem telah memenuhi persyaratan pengguna, memvalidasi kelayakan sistem dalam lingkungan produksi, dan mendapatkan persetujuan pengguna atas kecocokan dan kinerja sistem.

5. # **LINGKUNGAN USER ACCEPTANCE TEST**

   berisi environment testing berserta dengan ip address. contoh sta

   # *Berisi environment testing beserta dengan IP Address. Contoh: Staging Environment 192.168.100.1*

6. # **PERSIAPAN PENGUJIAN**    *Berisi persiapan yang perlu dilakukan sebelum memulai User Acceptance TEST misalnya adanya pelatihan pengguna* 

7. # **SKENARIO DAN HASIL PENGUJIAN**

1. \[Nama Project\]  
   Modul: \[Modul\]  
   Tanggal Pengujian: \[Tanggal Pengujian\]  
   

| No. | Skenario | Langkah-Langkah | Hasil yang diharapkan | Hasil yang dicapai | Keterangan |
| :---: | :---: | :---: | :---: | :---: | :---: |
|  |  |  |  |  |  |

8. # **KRITERIA KEBERHASILAN**

   

| No. | Kriteria Keberhasilan | Metrik Pengukuran | Target |
| :---: | :---: | :---: | :---: |
|  |  |  |  |

9. # **PENCATATAN MASALAH DAN TINDAK LANJUT**    *Berisi mekanisme pelaporan untuk mencatat masalah atau bug yang ditemukan selama pengujian.* 

   *Contoh mekanisme pelaporan yang menggunakan tabel tabel pencatatan* *masalah atau bugs*

   

| No. | Deskripsi Masalah | Prioritas | Tindak Lanjut | Tingkat Kesulitan | Status | Catatan Developer |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: |
|  |  |  |  |  |  |  |

10. # **EVALUASI DAN PENILAIAN**

    Berisi hasil evaluasi dan penilaian User Acceptance Test, termasuk pengumpulan umpan balik pengguna, analisis masalah atau bug yang teridentifikasi, dan tindakan perbaikan yang diperlukan.

11. # **LAMPIRAN**

    Berisi tentang dokumen pendukung seperti diagram, jadwal rinci, anggaran yang lebih terperinci, atau informasi tambahan yang relevan

**Approval**

| Disiapkan Oleh: |  |  |
| :---- | :---- | :---- |
|  |  |  |
| Jabatan | : | QA Engineer |
| Nama | : | Fiky Anggra Sepriaji |
| Tanggal | : |  |

| Disiapkan Oleh: |  |  |  |  |  |
| :---- | :---- | :---- | :---- | :---- | :---- |
|  |  |  |  |  |  |
| Jabatan | : |  | Jabatan | : |  |
| Nama | : |  | Nama | : |  |
| Tanggal | : |  | Tanggal | : |  |

| Disiapkan Oleh: |  |  |  |  |  |  |  |  |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- |
|  |  |  |  |  |  |  |  |  |
| Jabatan | : |  | Jabatan | : |  | Jabatan | : |  |
| Nama | : |  | Nama | : |  | Nama | : |  |
| Tanggal | : |  | Tanggal | : |  | Tanggal | : |  |

[image1]: <data:image/png;base64,>